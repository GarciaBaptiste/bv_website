...
function setup() {
  ...
  gui
    .add(graphicSettings[graphicSettingsIndex], "shadowMap", true)
    .onFinishChange(changeParameters);
  gui
    .add(graphicSettings[graphicSettingsIndex], "mapSize", 16, 2048, 16)
    .onFinishChange(changeParameters);
  gui
    .add(graphicSettings[graphicSettingsIndex], "bias", -10000, 10000, 1)
    .onChange(changeParameters);
  gui.add(requiredTime, "time", 0, 50, 0.1).onChange(updateAnimPosition);
  stats = new Stats();
      ...
      gltf.scene.traverse(function (object) {
        if (object.isMesh) {
          ...
          if (object.type === "PointLight") {
            ...
            object.intensity = object.intensity / 10;
          }
...
function initScene() {
  ...
  mixer.setTime(requiredTime.time);
  // if (!mobileDevice) {
...
function testLowerSettings() {
  if (graphicSettingsIndex > 0) {
    ...
    for (let i = 0; i < worldScene.children[1].children.length; i++) {
      resetGraphicSettings(worldScene.children[1].children[i]);
    }
  }
}
...
function resetGraphicSettings(elemToTest) {
  if (elemToTest.type === "PointLight") {
    ...
    elemToTest.shadow.mapSize.height =
      graphicSettings[graphicSettingsIndex].mapSize;
    elemToTest.shadow.bias =
      graphicSettings[graphicSettingsIndex].bias * 0.00001;
...
const keyFrames = [0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80];
